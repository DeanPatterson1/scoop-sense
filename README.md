...
| `scripts/apply-asins.js` | Rewrites `affiliateUrl` from an Amazon search URL to the listing itself. Checks the ASIN's shape, refuses an ASIN already claimed by another product, and requires a `listingTitle` naming the brand — the evidence the ASIN was actually looked at: `node scripts/apply-asins.js <json-file>`. |
| `scripts/check-affiliate-links.js` | Opens every direct link and reads the listing's own title, reporting anything that no longer names the right brand and product. Run it quarterly with the label re-verification — direct links rot as products are discontinued and relisted: `node scripts/check-affiliate-links.js`. |
...
| `scripts/normalize-product-images.py` | Converts new lead images to the house format (960×720 JPEG on white, contained not cropped) and repoints `imageUrl` at the file it wrote. `--all-non-jpg` sweeps everything `integrate-images.js` left behind; safe to re-run. |
| `README.md` | This file. |
...
The Amazon Associates tracking ID is **`thescoopsense-20`**, and 194 of the 201 `affiliateUrl`s in `data/products.js` carry it on a direct `/dp/` listing link. The category files and product pages are generated from that one file, so a tag change is a single find-and-replace there followed by `node scripts/build-product-pages.js` — never edit `products/*.html` or `data/by-category/*.js` by hand. `scripts/integrate-research.js` rejects any new product whose `affiliateUrl` does not carry the tag.

**Seven products have no US Amazon listing and link to the brand's own product page instead** — plain links that earn nothing until the matching brand program is joined: Mother Bucker (Bucked Up), Transparent Labs Intra, three 1st Phorm products (Micronized Creatine, Essential Amino Acids, Level-1), Xwerks Motion, and Applied Nutrition BCAA Amino Hydrate (a UK store). The page copy adapts by itself: the retailer name is derived from the link's host, and the buy box says "See the Bucked Up listing" rather than promising Amazon reviews. Program facts checked July 2026: Bucked Up runs an ambassador program at `promo.buckedup.com/ambassador` advertising up to 30% commission; 1st Phorm's "Legionnaire" program at `1stphorm.com/pages/affiliate` advertises up to 25%; Xwerks takes applications at `xwerks.com/pages/affiliate-program` at a stated 30%; Transparent Labs' own affiliates page is a dead link, so their program is reached through an affiliate network account (they historically ran on ShareASale, now part of Awin). Once accepted anywhere, replace that product's `affiliateUrl` with the program's tracked link — nothing else needs editing.
...
Sequencing is forced, and it is worth knowing before you start applying. Brand programs review a site's traffic before approving it, and one with none is rejected or ignored — so Amazon is the bootstrap that funds the traffic that qualifies you for the better programs, not the destination. The catalog is concentrated enough to make this tractable: 106 brands across 201 products, but the top twenty brands cover close to half of them, led by Optimum Nutrition at 10, Nutricost at 8, and Transparent Labs and Kaged at 7 each. Sixty-three brands carry exactly one product and are not worth an application.

**Links go to the listing, not to a search.** A search URL makes the reader hunt for the right tub and can surface a reseller, the wrong size, or a competitor's sponsored slot, so each `affiliateUrl` is `https://www.amazon.com/dp/<ASIN>?tag=thescoopsense-20`. Amazon shows the flavour picker on any child ASIN, so linking one flavour still lands the reader on the whole product.

An ASIN is arbitrary and cannot be derived from the product name — a guessed one silently sends a reader to somebody else's tub, which is the worst failure this site has. Every one has to come from a listing somebody opened and matched against the catalog's brand, variant and serving count. `scripts/apply-asins.js` applies them and refuses a batch that fails its checks; `scripts/check-affiliate-links.js` then reads each listing's real title back and is the thing that actually catches a wrong one. Watch for near-identical siblings, which is where these go wrong: C4 Original against C4 Sport against C4 Ultimate, Pre-Kaged against Pre-Kaged Elite, Bulk against Bulk Black, and the same tub sold in 1lb, 2lb and 5lb versions under different ASINs.
...
**Every lead image is stored in one house format** — 960×720 JPEG, product centred on white — so all 201 cards sit on the same stage and the grid reads as one set rather than a scrapbook of retailer photography at eight aspect ratios. Brand CDNs serve PNGs with alpha, WebP, squares and tall bottle shots; `scripts/normalize-product-images.py` converts whatever was downloaded, compositing transparency onto white and *containing* the image rather than cropping it to fill (a tub that loses its lid to a crop is a different tub to a reader scanning the grid). Run it after `integrate-images.js`, which does not normalise. A photo shot on a dark or coloured sweep survives that conversion as a dark rectangle among the white ones, so prefer another of the seller's images when one exists.
